...
- **"완료" 전에 실제로 검증**: 화면 스크린샷에만 의존하지 말고 **가능하면 코드를 직접 호출해서
  재현·검증**한다(예: `agent.run_agent()`를 스크립트로 직접 부르기, `curl`로 API 직접 때리기,
  DB 직접 조회). 이 방식을 쓴 사례가 화면 스크린샷 왕복보다 훨씬 빠르고 확실하게 원인을 잡았다.
  - 이전에 여기 적혀 있던 "AI은행원 iframe은 브라우저 자동화 클릭/타이핑이 안 먹는다"는 제약은
    **더 이상 사실이 아니다**(2026-08-26 정정). Playwright로 중첩 frame에 진입하면
    (`iframe[title="AI 금융상담 은행원"]` → `iframe[title="streamlitApp"]`) 채팅 입력·체크박스·
    PIN 입력·버튼 클릭이 전부 동작한다. 단 ① Streamlit 체크박스는 `<input>`을 직접 클릭하면
    오버레이가 pointer event를 가로채므로 **라벨 텍스트를 클릭**해야 하고 ② `st.text_input`은
    값 확정을 위해 **Enter를 눌러야** 한다.
  - iframe 임베드 성공 여부를 `onload` 이벤트로 판정하지 말 것 — **에러 페이지가 떠도 발생**한다.
    실제 렌더링을 스크린샷으로 확인해야 한다.
