...
> 관리자 계정(`admin`)은 개발자 전용이라 공개하지 않습니다. 방문자용 계정을 따로 둔 이유는 두 가지입니다 — 실제 관리자 아이디를 밖으로 내보내지 않고, 개발자가 확인할 때 쓰는 계정과 방문자 계정이 **잔액·일일 이체한도·거래내역을 공유하지 않게** 하려고입니다(DB가 전역 공유 상태라 같은 계정을 쓰면 서로의 화면이 바뀝니다).
>
