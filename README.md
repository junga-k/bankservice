...
### **https://bankservice-six.vercel.app/#auth**

**아이디에 `reviewer` 를 입력하면 비밀번호가 자동으로 채워집니다.** 위 링크로 들어가면 안내 카드가 있는 로그인 화면에 바로 도착합니다.

`reviewer` 하나로 방문자 기능과 백오피스를 전부 보실 수 있습니다.

| 구분 | 아이디 | 비밀번호 | 볼 수 있는 것 |
|---|---|---|---|
| **전체 둘러보기** | `reviewer` | 로그인 화면에서 자동 입력 | AI은행원 · 계좌조회 · 이체 · 상품안내 · 마이페이지 · 고객센터 + **백오피스 9개 메뉴** |
| 일반 사용자 | `demo` | `demo1234` | 위에서 백오피스를 뺀 나머지(권한이 낮은 화면을 따로 보고 싶을 때) |

> 관리자 계정(`admin`)은 개발자용이라 위 표에 싣지 않았습니다(비밀번호 기본값은 `seed_bank.py`에 있고, 라이브 값 교체는 남은 작업입니다). 방문자용 계정을 따로 둔 이유는 두 가지입니다 — 실제 관리자 아이디를 밖으로 내보내지 않고, 개발자가 확인할 때 쓰는 계정과 방문자 계정이 **잔액·일일 이체한도·거래내역을 공유하지 않게** 하려고입니다(DB가 전역 공유 상태라 같은 계정을 쓰면 서로의 화면이 바뀝니다).
>
> 이체 비밀번호는 여기 적지 않습니다 — **이체 확인 단계에서 화면에 안내**됩니다. 로그인 비밀번호와 이체 PIN은 서로 다른 인증 수단이라 한자리에 모아두지 않았습니다.
...
| 이체 | `lookup_recipient` | 받는 계좌의 예금주·은행·수수료 확인 |
...
사용자: "김철수한테 5만원 보내줘"
   ↓
LLM  : propose_transfer 호출 → 예금주·수수료 확인한 '제안'만 반환 (실행 수단 없음)
   ↓
UI   : 확인 카드 렌더 (예금주 · 금액 · 보내는 분 계좌 선택 · 신규 수취계좌 경고 · 이체 시점)
   ↓
사람 : "이체 확인" → 모달에서 체크박스 확인 + 이체 비밀번호 6자리 입력
   ↓
...
최종 확인(체크박스 + 이체 비밀번호)을 **모달로 분리**한 것도 같은 의도입니다. 카드 안에 폼 필드로 섞여 있으면 PIN이 입력란 하나로 묻히지만, 모달로 빼면 차단성 인증 이벤트로 읽혀 "돈이 움직이기 전에 사람이 한 번 막는다"는 설계가 화면에 드러납니다. 실패 시 모달은 닫히지 않고 그 안에서 사유를 보여줘 바로 재시도할 수 있습니다.
...
### 이체 수수료는 면제 — 단, 로직은 남겨뒀습니다

현재 정책은 수수료 0원입니다. 국내에서는 이미 면제가 시장 표준이라, 신규 서비스가 건당 수수료를 받으면 재방문할 이유가 약해집니다. 다만 **수수료 로직 자체는 지우지 않았습니다** — 잔액 검사(`amount + fee`), 별도 `이체수수료` 거래내역, 백오피스 이체정책 편집이 모두 그대로 있고 정책값만 0입니다. 백오피스에서 수수료를 올리면 이체 화면 안내가 "면제"에서 실제 금액으로 자동 전환되고, 강조 색도 빠집니다.

화면 문구를 하드코딩하지 않은 이유가 여기 있습니다. 한도·수수료는 백오피스에서 바뀔 수 있어서, 이체 화면 안내와 "이체 후 잔액" 미리보기가 모두 `GET /api/me/limits`를 읽습니다(예전에는 `타행 수수료 500원`이 마크업에 박혀 있어 정책과 어긋났습니다).

### 드롭다운을 직접 만든 이유

네이티브 `<select>`의 옵션 목록은 **OS가 그리는 메뉴**라 CSS로 크기·폰트·위치를 제어할 수 없습니다(macOS에서 목록이 화면 가운데에 아주 크게 떴습니다). `<option>` 안에서는 flexbox·정렬도 동작하지 않아 "은행 · 계좌번호 · 잔액(오른쪽 정렬)" 같은 행 구성이 불가능하고, 드롭다운 화살표는 `background-image`라 오른쪽 테두리에만 붙습니다.

그래서 목록 UI만 직접 그리는 재사용 컴포넌트(`.cs`)를 만들었습니다. **네이티브 `<select>`는 지우지 않고 숨겨서 값 저장용으로 남겼습니다** — 이체 폼 여러 곳이 `.value`를 읽고 쓰기 때문에 그 인터페이스를 유지하고, 선택 시 `change` 이벤트를 발생시켜 기존 핸들러를 그대로 재사용합니다.
...
| 내 계좌 | 잔액·거래내역 조회, 3단계(입력→확인→완료) 이체 · 이체 수수료 면제 |
...
.venv/bin/python seed_bank.py      # 사용자·계좌·거래·이체 (reviewer/reviewer1234, demo/demo1234, admin/admin1234)
...
| `JWT_SECRET` | JWT 서명 키. **배포 환경에서는 반드시 설정** — 미설정 시 저장소에 있는 폴백 상수를 쓰게 되고, 공개 저장소이므로 누구나 `role:"admin"` 토큰을 위조할 수 있다 |
| `DEMO_LOGIN` | 로그인 화면이 안내할 공개 계정. `아이디:비밀번호` 형식(예: `reviewer:...`). 미설정 시 안내 카드와 자동 입력이 아예 렌더되지 않는다(로컬 개발 기본값) |
| `DEMO_TRANSFER_PIN` | 이체 확인 단계에서 안내할 데모 이체 비밀번호. 로그인한 계정이 `DEMO_LOGIN`의 계정일 때만 노출된다 |
| `REVIEWER_PASSWORD` / `REVIEWER_TRANSFER_PIN` | `seed_bank.py`가 만드는 공개 계정의 비밀번호·이체 PIN (미설정 시 로컬 기본값) |
| `ADMIN_PASSWORD` | 관리자 계정 비밀번호. 라이브에서는 공개하지 않는 값으로 설정한다. ⚠️ 이미 존재하는 사용자의 비밀번호는 시드가 덮어쓰지 않으므로, 최초 1회는 마이페이지>보안에서 직접 변경해야 한다 |
...
`batch_test.py`는 10개 카테고리 999문항을 동시 실행하면서 Phoenix로 트레이스를 남기고, 응답 성공률·지연과 함께 **정답 채점**(수학은 정답 문자열 exact 비교, 나머지는 LLM-as-judge)까지 수행합니다.

최근 실행 결과 (gpt-4o-mini, 999문항 / 171초):
...
| 정확도 | **96.3%** (950/987 채점) |
| 평균 지연 | 1,038 ms |
| 중앙값 / p95 | 863 ms / 1,881 ms |

카테고리별 정확도 — 수학·과학·영어·파이썬코딩 100%, 경제 99.0%, 세계지리 97.0%, 한국역사 94.9%, 상식 93.9%, 문화예술 93.0%, **논리 84.5%**. 논리 추론이 뚜렷하게 약한 구간으로 드러났습니다.

> 문항이 은행 업무가 아니라 일반 지식인 이유는, 이 배치가 도메인 정확도 평가가 아니라 **응답 파이프라인의 안정성과 지연 특성을 재기 위한 것**이기 때문입니다. 은행 업무 답변의 품질은 사용자 피드백(좋아요/싫어요 + 사유) 집계로 따로 봅니다. 채점되지 않은 12건은 판정 모델이 O/X 외의 응답을 낸 경우입니다.
...
- **Kafka·Elasticsearch·Redis는 배포 환경에 두지 않았습니다** — 셋 다 로컬에서만 동작합니다. 호스팅하지 않은 건 미완이 아니라 판단입니다. 이체는 `KAFKA_DISABLED` 동기 폴백이 정상 동작하고(라이브에서 실제 이체로 확인), 큐를 붙이려면 상시 실행 컨슈머를 둘 곳이 따로 필요합니다. RAG·시맨틱 캐시는 애초에 **로그인 사용자가 쓰는 에이전트 경로가 아니라 일반 대화 경로**에 붙어 있어서, 라이브에 올려도 실행되지 않습니다. 되살리려면 에이전트 경로에 태우는 코드 변경이 선행돼야 합니다.
...
- **토큰을 localStorage에 저장** — XSS에 노출될 수 있는 구조입니다. httpOnly 쿠키로 옮기는 것이 실서비스 기준이고, 데모 편의로 현재 방식을 유지했습니다. JWT 서명 키는 `JWT_SECRET` 환경변수로 분리했습니다(미설정 시 로컬 개발용 폴백).
- **AI 은행원에 토큰이 URL로 전달됩니다** — 사이트가 챗봇 iframe 주소에 `?token=`으로 JWT를 넘깁니다. 서드파티 호스트(Streamlit Community Cloud)에 토큰이 닿고 referrer·로그에 남을 수 있어, 실서비스라면 `postMessage` 등으로 바꿔야 합니다.
